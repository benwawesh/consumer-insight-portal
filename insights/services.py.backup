    ...
    def fetch_offers(self):
        ...
        try:
            ...
            if isinstance(data, dict):
                # The API returns offers as a dict with offer_id as keys
            ...
            logger.info(f"Fetched {len(offers)} offers from CPAGrip")
            return offers
            
        except requests.exceptions.RequestException as e:
            logger.error(f"Failed to fetch offers from CPAGrip: {e}")
            return None
        except Exception as e:
            logger.error(f"Unexpected error fetching offers: {e}")
            return None
